...
{
    $sql = "SELECT * FROM about_us WHERE id_about=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('i', $about_id);
    $stmt->execute();
    $result = $stmt->get_result();

    $about_us = $result->fetch_all(MYSQLI_ASSOC);
    $stmt->close();
    return $about_us;
}

function editAboutUs($conn, $about_id)
{
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $sql = "UPDATE about_us SET title=?,description=? WHERE id_about=?";

        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssi",  $about_title, $description, $about_id);
...
<head>
    <!-- Title -->
    <title>A-1 uniforms - Edit About_Us</title>
                    ...
                    <div class="col-xl-12">
                        <div class="card  card-bx m-b30">
                            <div class="card-header bg-primary">
                                <h6 class="title text-white">Edit About_Us</h6>
                            </div>

                            <?php foreach ($about_us as $about_us) {
                            ?>
                                <form class="profile-form" action="edit-about_us.php?about_id=<?php echo $about_us['id_about']; ?>" method="post" onsubmit="return validateForm();">
                                    <div class="card-body">
                                        <div class="row">
                                        <div class="col-sm-12 mb-3">
											<label class=" form-label required">Title</label>
											<input type="text" name="title" class="form-control" placeholder="Enter name category..." value="<?php echo $about_us['title'] ?>">
										</div>
                                       

                                            <div class="col-sm-12 mb-3">
                                                <label class="form-label required">Description</label>
                                                <input type="text" name="description" class="form-control" placeholder="Enter description..." value="<?php echo $about_us['description']; ?>" >
                                            </div>
                                           
                                        </div>
                                    </div>
                                    <div class="card-footer justify-content-end">
                                        <button class="btn btn-primary">Update About_Us</button>
                                    </div>
                                </form>
                            <?php }
                            ?>
