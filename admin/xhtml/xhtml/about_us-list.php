...
{
	$sql = "SELECT COUNT(*) FROM about_us";
...
function getAboutUsByTitle($conn)
{
	if (isset($_GET['term'])) {
		$searchTerm = '%' . $_GET['term'] . '%';

		$sql = "SELECT * FROM about_us WHERE title LIKE ?";
		$stmt = $conn->prepare($sql);
		$stmt->bind_param('s', $searchTerm);
		$stmt->execute();
		$result = $stmt->get_result();
		$aboutUs_Search = $result->fetch_all(MYSQLI_ASSOC);
...
$aboutUs_Search = getAboutUsByTitle($conn);
								...
								<div class="card-body py-3 py-md-2">
									<div class="row align-items-center">
										...
										<div class="col-md-7 text-md-end">
											<li class="nav-item dropdown notification_dropdown">
												<form action="about_us-list.php" method="GET">
							...
							<div class="table-responsive">
								<table id="example5" class=" display mb-4 table-responsive-xl dataTablesCard fs-14">
									...
									<tbody>
										...
										if (!empty($aboutUs_Search)) {
											foreach ($aboutUs_Search as $aboutUs_Search) {
										?>
												<tr>
												<td>
													<div class="form-check custom-checkbox">
												...
												<td>
													<p><?php echo $aboutUs_Search['title'] ?></p>
												</td>
												<td>
													<p><?php echo $aboutUs_Search['description'] ?></p>
												</td>
