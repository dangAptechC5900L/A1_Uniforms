...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['contact_id'])) {
	$about_id = $_POST['contact_id'];

	// Đảo ngược trạng thái của khách hàng trong cơ sở dữ liệu
	$sql = "UPDATE contact SET isDeleted = 1 - isDeleted WHERE contact_id = ?";
...
function getTotalContactUs($conn)
{
...
function getContactUsByTitle($conn)
{
	if (isset($_GET['term'])) {
		$searchTerm = '%' . $_GET['term'] . '%';

		$sql = "SELECT * FROM contact WHERE subject LIKE ?";
		$stmt = $conn->prepare($sql);
		$stmt->bind_param('s', $searchTerm);
		$stmt->execute();
		$result = $stmt->get_result();
		$contactUs_Search = $result->fetch_all(MYSQLI_ASSOC);
...
$contactUs_Search = getContactUsByTitle($conn);
$contact_us = getAllContactUs($conn);

$rows = getTotalContactUs($conn);
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
	<!-- Title -->
	<title>A1-Uniforms - Contact_Us</title>
...
<body>
				...
				<div class="container-fluid">
					<div class="row mb-5 align-items-center">
						<!-- <div class="col-xl-3 mb-4 mb-xl-0">
							<a href="add-about_us.php" class="btn btn-primary light btn-lg d-block rounded shadow px-2">+ New About Us</a>
						</div> -->
						<div class="col-xl-9">
							<div class="card m-0 ">
									...
									<div class="row align-items-center">
										...
										<div class="col-md-7 text-md-end">
											<li class="nav-item dropdown notification_dropdown">
												<form action=" contact_us-list.php" method="GET">
													<div class="input-group search-area" style="margin-left:200px">
														<input type="text" class="form-control" name="term" placeholder="Search title here...">
										...
										?>
												<tr>
													...
													<td>
														<div class="btn-group">
															<form action="contact_us-list.php" method="post">
																<button type="submit" name="contact_id" value="<?php echo $contact_Us_Search['contact_id']; ?>" class="btn <?php echo $contact_Us_Search['isDeleted'] == 0 ? 'btn-success' : 'btn-secondary'; ?> rounded">
																	<?php echo $contact_Us_Search['isDeleted'] == 0 ? "Processing" : "Pending"; ?>
																</button>
															</form>
														</div>
													...
													<td>
														...
														<div class="d-flex">
															<a href="showContactUs.php?contact_id=<?php echo $contact_Us_Search['contact_id']; ?>" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
											...
											?>
												<tr>
													...
													<td>
														<div class="btn-group">
															<form action="contact_us-list.php" method="post">
																<button type="submit" name="contact_id" value="<?php echo $contact_Us['contact_id']; ?>" class="btn <?php echo $contact_Us['isDeleted'] == 0 ? 'btn-success' : 'btn-secondary'; ?> rounded">
																	<?php echo $contact_Us['isDeleted'] == 0 ? "Processing" : "Pending"; ?>
																</button>
															</form>
														</div>
													...
													<td>
														...
														<div class="d-flex">
															<a href="showContactUs.php?contact_id=<?php echo $contact_Us['contact_id']; ?>" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a>
    ...
    ***********************************-->
	...
	<script>
		...
		// Gán sự kiện click cho mỗi nút "edit"
		editButtons.forEach(function(button) {
			button.addEventListener('click', function() {
				// Lấy dữ liệu của hàng tương ứng
				var row = this.closest('tr');
				var title = row.querySelector('.title-column').innerText;
				var description = row.querySelector('.description-column').innerText;
				var status = row.querySelector('.status-column').innerText;

				// Hiển thị pop-up và điền dữ liệu vào các trường
		...
		// Đóng pop-up khi người dùng nhấp vào nút "Close"
